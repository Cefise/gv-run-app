...
(function () {
  ...
  /* E3 — simetria */
  .gv-symbig{display:flex;align-items:center;gap:12px;margin:6px 0 14px}
  .gv-symbig .num{font-size:34px;font-weight:800}
  .gv-symbig .arrow{color:var(--mut);font-size:20px}
  .gv-badge{display:inline-block;font-size:11px;font-weight:800;letter-spacing:.5px;
    border-radius:999px;padding:5px 12px;margin-top:2px}
  .gv-badge.ok{background:rgba(47,163,107,.16);border:1px solid rgba(47,163,107,.45);color:#CFF3E1}
  .gv-badge.warn{background:rgba(227,160,8,.14);border:1px solid rgba(227,160,8,.45);color:#F3DFAE}
  .gv-symrow{margin:10px 0}
  .gv-symrow .lbl{display:flex;justify-content:space-between;font-size:12.5px;margin-bottom:4px}
  .gv-symrow .lbl b{color:var(--txt)}
  .gv-track{height:9px;border-radius:6px;background:rgba(255,255,255,.07);overflow:hidden}
  .gv-fill{height:100%;border-radius:6px}
  `;
  ...
  /* ================= ETAPA 3 — SIMETRIA (aba Evolução) ================= */
  const TESTN = { balance: "Equilíbrio", calf: "Panturrilha", bridge: "Ponte de glúteo", stepdown: "Step-down", hop: "Salto unipodal" };

  // média de simetria (%) de uma avaliação + lista por teste
  function symOf(a) {
    if (!a || typeof sideValues !== "function" || typeof lsi !== "function" || !window.ASSESS) return null;
    const rows = [];
    ASSESS.forEach(t => {
      const r = a[t.id]; if (!r) return;
      const sv = sideValues(t, r); if (!sv || !sv.d || !sv.e) return;
      const L = lsi(sv.d, sv.e); if (!L) return;
      rows.push({ id: t.id, name: TESTN[t.id] || t.q, sym: L.sym, weak: L.weak });
    });
    if (!rows.length) return null;
    const mean = Math.round(rows.reduce((x, r) => x + r.sym, 0) / rows.length);
    return { mean, rows };
  }

  function injectSymCard() {
    const rep = document.getElementById("repAssess");
    if (!rep || document.getElementById("gvSym")) return;
    const host = rep.closest(".card") || rep;
    const card = document.createElement("div");
    card.className = "card";
    card.id = "gvSym";
    card.innerHTML = `
      <span class="tag">Simetria entre os lados</span>
      <h3>Seu corpo em equilíbrio</h3>
      <div class="gv-symbig" id="gvSymBig"></div>
      <div id="gvSymBadge"></div>
      <div id="gvSymRows" style="margin-top:12px"></div>
      <p style="font-size:11.5px;color:var(--mut);margin-top:12px;line-height:1.5">Simetria compara o seu lado mais fraco com o mais forte em cada teste. Diferenças grandes entre os lados merecem atenção no treino.</p>`;
    host.parentNode.insertBefore(card, host);
  }

  function updateSymCard() {
    if (!window.S) return;
    injectSymCard();
    const big = document.getElementById("gvSymBig");
    if (!big) return;

    const s0 = symOf(S.assess0);
    const s8 = symOf(S.assess8);
    const cur = s8 || s0;

    if (!cur) {
      big.innerHTML = `<span style="color:var(--mut);font-size:14px">Complete a avaliação para ver sua simetria aqui.</span>`;
      document.getElementById("gvSymBadge").innerHTML = "";
      document.getElementById("gvSymRows").innerHTML = "";
      return;
    }

    const colorFor = (v) => v >= 90 ? "var(--green)" : "var(--amber)";
    if (s8 && s0) {
      big.innerHTML = `<span class="num" style="color:${colorFor(s0.mean)}">${s0.mean}%</span>
        <span class="arrow">→</span>
        <span class="num" style="color:${colorFor(s8.mean)}">${s8.mean}%</span>
        <span style="color:var(--mut);font-size:12px">semana 0 → agora</span>`;
    } else {
      big.innerHTML = `<span class="num" style="color:${colorFor(cur.mean)}">${cur.mean}%</span>
        <span style="color:var(--mut);font-size:12px">simetria média · ${s8 ? "reavaliação" : "avaliação inicial"}</span>`;
    }

    const ok = cur.mean >= 90;
    document.getElementById("gvSymBadge").innerHTML =
      `<span class="gv-badge ${ok ? "ok" : "warn"}">${ok ? "✅ Simetria ideal" : "⚠️ Assimetria alta — atenção"}</span>`;

    document.getElementById("gvSymRows").innerHTML = cur.rows.map(r => `
      <div class="gv-symrow">
        <div class="lbl"><b>${r.name}</b><span style="color:${colorFor(r.sym)}">${r.sym}%${r.weak !== "—" ? ` <small style="color:var(--mut)">· lado ${r.weak.toLowerCase()} mais fraco</small>` : ""}</span></div>
        <div class="gv-track"><div class="gv-fill" style="width:${r.sym}%;background:${colorFor(r.sym)}"></div></div>
      </div>`).join("");
  }

  const _prevReport = window.renderReport;
  window.renderReport = function () {
    if (typeof _prevReport === "function") _prevReport();
    try { updateSymCard(); } catch (e) { console.error("gv-ui sym:", e); }
  };

  /* ================= ETAPA 3 — PERFIL (conta + sair) ================= */
  function injectProfileCard() {
    const prof = document.getElementById("vProfile");
    if (!prof || document.getElementById("gvAccount")) return;
    const card = document.createElement("div");
    card.className = "card";
    card.id = "gvAccount";
    card.innerHTML = `
      ...
      <h3 id="gvAccEmail">—</h3>
      <p style="color:var(--mut);font-size:13px">Seu progresso fica salvo na nuvem e sincroniza entre celular e computador.</p>
      <button class="btn ghost" style="margin-top:10px" onclick="GVCloud && GVCloud.logout()">Sair da conta</button>`;
    prof.insertBefore(card, prof.firstElementChild);
  }

  function updateProfileCard() {
    injectProfileCard();
    const el = document.getElementById("gvAccEmail");
    if (el && window.GVCloud && GVCloud.user && GVCloud.user.email) el.textContent = GVCloud.user.email;
  }

  const _prevShow = window.show;
  window.show = function (id) {
    if (typeof _prevShow === "function") _prevShow(id);
    if (id === "vProfile") { try { updateProfileCard(); } catch (e) {} }
  };

  /* ---------------- primeira pintura ---------------- */
  try { updateWidgets(); } catch (e) {}
  try { updatePainChart(); } catch (e) {}
  try { updateSymCard(); } catch (e) {}
